...
)
{
  ...
  /*Read in each line of calibration file*/
  /*Assign values in each line to calibration variables*/
...
void makePlot_Bvz()
{
  ...
  TLine *FM_start = new TLine(-82.55, 0.0,-82.55, 4.5);
  FM_start->SetLineStyle(2);
  FM_start->SetLineColor(kRed+1);
  FM_start->Draw();
  TLine *FM_end = new TLine(82.55, 0.0 , 82.55, 4.5);
  FM_end->SetLineStyle(2);
  FM_end->SetLineColor(kRed+1);
  FM_end->Draw();
  TLine *SC_start = new TLine(-508, 0.0,-508, 4.5);
  SC_start->SetLineStyle(1);
  SC_start->SetLineColor(kBlue);
  SC_start->Draw();
  TLine *SC_end = new TLine(508, 0.0 , 508, 4.5);
  SC_end->SetLineStyle(1);
  SC_end->SetLineColor(kBlue);
  SC_end->Draw();
  ...
  */
  ...
  l_FMscan->AddEntry( FM_start , "End of Magnet" , "l");
  l_FMscan->AddEntry( SC_start , "End of Superconductor" , "l");
  l_FMscan->Draw();
